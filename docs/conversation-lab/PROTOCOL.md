...
The current panel is **v2**, with seven frozen scenarios in
`docs/conversation-lab/testbed-v2.json`. Each speaker anchor is generated by
production's `_build_recipe_context` (title, category, and description), and
each scenario carries full `judge_recipe_facts` for checking technique claims
against the recipe method. The panel spans four categories, an accented title,
and short and long methods.
...
The original five-scenario **v1** remains at `docs/conversation-lab/testbed.json`
for interpreting earlier experiments. Its ingredient-list anchors predate the
2026-09-15 production change; selecting it prints a stale-anchor warning.
Record the panel version and path with each experiment: pre-v2 measurements
used v1, while current runs default to v2. Compare experiments within the same
panel version; changing the panel also changes the comparison baseline.

`ab --testbed` runs every scenario in the panel (bare flag defaults to the
v2 file; pass a path to use another one) instead of a single `--concept`
