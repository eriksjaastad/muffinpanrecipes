...
The current panel is **v3**, with seven frozen scenarios in
`docs/conversation-lab/testbed-v3.json`. Each speaker anchor is generated by
production's `_build_recipe_context` (card #7441), which includes title,
category, description, **and ingredient boundaries** that speakers can reference
without inventing ingredients. Each scenario carries full `judge_recipe_facts`
for checking technique claims against the recipe method, and structured
`recipe_data` for reproducibility audits. The panel spans four categories,
an accented title, and short and long methods.
...
- **W37 - Brazilian Pão de Queijo Bites** - party, accented title.
- **W11 - Make-Ahead Veggie & Sausage Egg Cups** - the oldest week with
dialogue, for range across the corpus's age.
- **W38 - Cardamom Cinnamon Spiral Bites** - sweet, 36 method steps;
the technique-fidelity regression scenario.

**v3** (`testbed-v3.json`) is the current anchor panel, built with #7441-aware
recipe context that includes ingredient boundaries speakers see in production.
**v2** (`testbed-v2.json`) is available for baseline comparison. It predates
the #7441 ingredient boundary and records what testbed measurements looked like
before ingredients were visible to speakers; loading v2 prints a warning that
results do not transfer to production. **v1** (`testbed.json`) is the
original five-scenario panel with pre-#7104 "Key ingredients:" anchors; selecting
it prints a stale-anchor warning. Record the panel version and path with each
experiment: compare experiments within the same panel version; changing the
panel also changes the comparison baseline.

`ab --testbed` runs every scenario in the panel (bare flag defaults to the
v3 file; pass a path to use another one) instead of a single `--concept`
...
(default 3 in testbed mode, so 21 pairs across all seven by default) and the
...
is current. Every panel load prints its version, filename, and scenario count;
saved reports include the testbed path. See `ab --help` for the exact flags.

## Lab judge dimensions

### Orientation evidence records

Every pairwise judge generation attempt records its rendered prompt, system
...
partial pairs retain any generation attempt, including malformed output.
...
Without a shared budget guard, `calls_used` counts judge orientations invoked
after the lab's cap checks. With the guard, it counts deltas in the ledger's
`generation_attempts`: token-count failures and budget rejections before
reservation are excluded and leave the failed pair in the run-level partial
report without a judge orientation record. A failed ledger snapshot is marked
