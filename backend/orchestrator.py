...
class RecipeOrchestrator:
    ...
    def produce_recipe(self, concept: str) -> tuple[Recipe, CreationStory]:
        ...
        try:
            ...
            # Note: dialogue generation is handled by run_pipeline_stage.py via
            # simulate_dialogue_week.run_simulation() — not the hardcoded fallback here.
    ...
    def _record_creative_dialogue(
        ...
        """Deprecated: dialogue is now generated by simulate_dialogue_week.run_simulation().

        This method previously added 4 hardcoded scripted lines to the pipeline context.
        It is retained for backward compatibility in case anything still calls it, but
        no longer appends messages — the simulator produces real LLM dialogue instead.
        """
        pass  # Real dialogue generated in run_pipeline_stage._generate_stage_dialogue()
