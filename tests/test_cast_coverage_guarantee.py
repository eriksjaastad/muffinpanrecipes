...
@pytest.mark.parametrize("day", DAY_ORDER)
def test_every_day_can_close_its_own_scene(day: str) -> None:
    """Every day budgets at least one turn more than it has cast members.
    ...
    nothing meaningful" — while scoring cast_coverage 5. Seating everyone is
    not the same as giving them a conversation.

    Sunday was fixed first (#7082) because it was blocking a live publish;
    tuesday, wednesday, thursday and friday followed (#7105) once it was clear
    the same arithmetic applied to them. A ledger test used to name those four
    as known-exposed — it is gone, because the exposure is.
    """
    low, _high = TICKS_RANGE[day]
    cast_size = len(participants_for_day(day))
    assert low > cast_size, (
        f"{day} budgets a minimum of {low} turns for {cast_size} characters; "
