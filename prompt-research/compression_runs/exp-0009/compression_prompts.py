...
def format_highlights(highlights: list[str]) -> str:
    ...
    # Build a "key decisions made" block for Sunday to reference or build on
    # Scan all highlights for sentences that sound like decisions/conclusions
    decision_markers = [
        "decided", "agreed", "confirmed", "settled", "chose", "locked in",
        "going with", "will use", "final", "approved", "consensus"
    ]
    decision_lines = []
    for h in highlights:
        parts = h.split(":", 1)
