...
def format_highlights(highlights: list[str]) -> str:
    ...
    # Build a focused Sunday agenda from the most actionable open threads
    sunday_agenda_items = []
    for h in highlights:
        h_lower = h.lower()
        if "still open:" in h_lower:
            idx = h_lower.index("still open:")
            open_text = h[idx + len("still open:"):].strip().rstrip(".")
            sunday_agenda_items.append(f"- Resolve: {open_text}")

    # Add a closing reflection prompt to encourage natural wrap-up
    sunday_agenda_items.append(
        "- Sunday is the final session: characters should reflect on the week's journey and land on concrete decisions."
    )

    sunday_agenda = "\n".join(sunday_agenda_items) if sunday_agenda_items else ""
