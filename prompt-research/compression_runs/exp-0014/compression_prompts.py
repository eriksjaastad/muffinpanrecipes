...
def format_highlights(highlights: list[str]) -> str:
    ...
    # Build a concise "Sunday agenda" that explicitly names what needs resolution
    # This gives the model a clear action-oriented frame for Sunday's dialogue
    sunday_agenda_items = []
    for h in highlights:
        h_lower = h.lower()
        if "still open:" in h_lower:
    ...
    sunday_agenda = ""
    if sunday_agenda_items:
        sunday_agenda = "\n\nSUNDAY AGENDA (unresolved from the week):\n" + "\n".join(sunday_agenda_items[:4])
