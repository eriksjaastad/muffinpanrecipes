...
def format_highlights(highlights: list[str]) -> str:
    ...
    for h in highlights:
        ...
        if len(parts) < 2:
            continue
        detail = parts[1].strip()
        # Extract first-name mentions with context
        words = detail.split()
        for i, word in enumerate(words):
            # Names are capitalized and not at sentence boundaries typically
            clean_word = word.rstrip(".,;:")
            if clean_word and clean_word[0].isupper() and len(clean_word) > 2 and clean_word.isalpha():
                # Grab a small context window around the name
                start = max(0, i - 1)
                end = min(len(words), i + 6)
                snippet = " ".join(words[start:end])
                if clean_word not in character_positions:
                    character_positions[clean_word] = []
                if len(character_positions[clean_word]) < 2:
                    character_positions[clean_word].append(snippet)

    # Only include characters with meaningful mentions (2+ words context)
    char_lines = []
    # Common non-name capitalized words to skip
    skip_words = {
