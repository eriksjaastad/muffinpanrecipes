...
def format_highlights(highlights: list[str]) -> str:
    ...
    # Build the final injection with a strong continuity nudge
    ...
    if char_lines:
        char_block = "CHARACTER POSITIONS THIS WEEK:\n" + "\n".join(char_lines[:6])
        base_injection = base_injection + "\n\n" + char_block

    # Add a direct instruction to open Sunday with a callback to a specific earlier moment
    if highlights:
        # Pick the most recent highlight with a named person and specific detail
        callback_seed = None
        for h in reversed(highlights):
            parts = h.split(":", 1)
            if len(parts) == 2:
                detail = parts[1].strip()
                # Look for a sentence with a name and a specific detail
                sentences = [s.strip() for s in detail.split(".") if len(s.strip().split()) >= 5]
                for sentence in sentences:
                    words = sentence.split()
                    has_name = any(
                        w.rstrip(".,;:")[0].isupper() and w.rstrip(".,;:").isalpha() and len(w.rstrip(".,;:")) > 2
                        and w.rstrip(".,;:") not in {
                            "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday",
                            "Still", "The", "This", "That", "It", "We", "They", "He", "She",
                        }
        ...
        if callback_seed:
            continuity_nudge = (
                "\n\nSUNDAY OPENING HOOK:\n"
                f"Start Sunday's conversation by having a character directly reference or react to this: \"{callback_seed}\"\n"
                "This grounds the dialogue in the week's history from the very first line."
            )
            base_injection = base_injection + continuity_nudge
