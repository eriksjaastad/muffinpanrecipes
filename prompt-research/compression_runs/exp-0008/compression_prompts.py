...
def format_highlights(highlights: list[str]) -> str:
    ...
    # Build a running-debate block: track the arc of disagreement across the week
    # Find pairs of highlights where the same topic shifted or stayed contested
    debate_arc_lines = []
    all_details = []
    for h in highlights:
        parts = h.split(":", 1)
        if len(parts) == 2:
            day_label = parts[0].strip()
            detail = parts[1].strip()
            all_details.append((day_label, detail))

    # Look for recurring names or topics across multiple days
    topic_days: dict[str, list[str]] = {}
    for day_label, detail in all_details:
        words = detail.split()
        for word in words:
            clean = word.rstrip(".,;:?!")
            if len(clean) >= 4 and clean[0].isupper() and clean.isalpha():
                skip = {
                    "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday",
                    "Still", "The", "This", "That", "They", "With", "From", "After", "Before",
                    "Also", "Both", "Each", "Some", "More", "Less", "Much", "Many", "Most",
                }
                if clean not in skip:
                    if clean not in topic_days:
                        topic_days[clean] = []
                    if day_label not in topic_days[clean]:
                        topic_days[clean].append(day_label)

    # Topics that appear across 3+ days are recurring debates worth noting
    recurring = [(topic, days) for topic, days in topic_days.items() if len(days) >= 3]
    if recurring:
        recurring.sort(key=lambda x: -len(x[1]))
        top_recurring = recurring[:3]
        for topic, days in top_recurring:
            debate_arc_lines.append(f"- {topic} came up across {', '.join(days)}")

    result = INJECTION_TEMPLATE.format(
    ...
    if debate_arc_lines:
        result += "\n\nRECURRING THEMES (worth closing out on Sunday):\n" + "\n".join(debate_arc_lines)
