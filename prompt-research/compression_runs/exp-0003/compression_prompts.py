...
def format_highlights(highlights: list[str]) -> str:
    ...
    if open_thread_lines:
        ...
        open_threads_text = (
            recent_anchor + "\n"
            "- No major unresolved disagreements - Sunday can reflect on the full week's journey and finalize decisions."
        )
    else:
        open_threads_text = "- No major unresolved disagreements - Sunday can focus on final decisions and reflection."

    full_injection = INJECTION_TEMPLATE.format(
