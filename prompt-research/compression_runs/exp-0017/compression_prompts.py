...
def format_highlights(highlights: list[str]) -> str:
    ...
    # This is the ONE change: instead of extracting from summaries (which is noisy),
    # we directly list the most recent day's named positions as a "going into Sunday" block.
    # This gives the model a cleaner, more actionable character-position signal.
        ...
        for sentence in sentences:
            ...
            for i, word in enumerate(words):
                ...
                skip_words = {
                    ...
                    "Key", "Also", "Both", "All", "Some", "More", "Less",
                }
    ...
    # Build the final injection block
    base_injection = INJECTION_TEMPLATE.format(
    ...
    if char_lines:
        ...
        # Add a Sunday-specific prompt: what each character might want to resolve
        resolution_prompt = (
            "\n\nSUNDAY GOAL: Each character should arrive with a point of view shaped by the week's "
            "debates. The conversation should feel like a natural conclusion - decisions get made, "
            "tensions get resolved or acknowledged, and the recipe direction gets finalized. "
            "Reference specific earlier moments by name (e.g. 'Like Margaret said on Tuesday...' "
            "or 'We never settled the question about...')."
        )
        return base_injection + char_block + resolution_prompt
    ...
    return base_injection + resolution_prompt