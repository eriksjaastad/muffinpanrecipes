...
def format_highlights(highlights: list[str]) -> str:
    ...
    for h in highlights:
        ...
        for sentence in sentences:
            ...
            # Only look at the first word of each sentence as a potential name
            if not words:
                continue
            ...
            if (first_word and first_word[0].isupper() and len(first_word) > 2
                    and first_word.isalpha() and first_word not in skip_words):
                name = first_word
                end = min(len(words), 8)
                snippet = " ".join(words[:end]).rstrip(".,;:")
                if name not in character_positions:
                    character_positions[name] = []
                if len(character_positions[name]) < 2 and len(snippet.split()) >= 3:
                    character_positions[name].append(snippet)
