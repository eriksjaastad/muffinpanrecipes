...
def format_highlights(highlights: list[str]) -> str:
    ...
    # Build a "key decisions made" block from the most concrete sentences across the week
    # This gives Sunday characters a clear list of settled items to reference or revisit
    decision_markers = [
        "decided", "agreed", "confirmed", "settled", "chose", "locked in",
        "went with", "will use", "finalized", "voted", "consensus"
    ]
    decision_lines = []
    for h in highlights:
        parts = h.split(":", 1)
        ...
        for sentence in sentences:
            ...
            if any(marker in sentence_lower for marker in decision_markers):
                if len(sentence.split()) >= 5:
                    decision_lines.append(f"- ({day_label}) {sentence}")
                    break  # one decision per day is enough
