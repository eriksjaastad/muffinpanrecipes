...
def format_highlights(highlights: list[str]) -> str:
    ...
    # Build a "tensions and turning points" block highlighting the most dramatic
    # shifts or conflicts across the week — gives Sunday dialogue richer material
    tension_lines = []
    tension_markers = [
        "pushed back", "disagreed", "argued", "insisted", "refused",
        "challenged", "objected", "countered", "but", "however",
        "instead", "rather", "opposed", "conflict", "tension",
        "debated", "disputed", "clashed", "resisted", "rejected"
    ]
    for h in highlights:
        parts = h.split(":", 1)
        if len(parts) < 2:
        ...
        for sentence in sentences:
            ...
            if any(marker in s_lower for marker in tension_markers):
                tension_lines.append(f"- {day_label}: {sentence}")
                break  # one tension per day is enough

    if tension_lines:
        tension_block = "KEY TENSIONS THIS WEEK:\n" + "\n".join(tension_lines[:4])
    else:
        tension_block = None

    # Assemble final output
    base = INJECTION_TEMPLATE.format(
    ...
    extras = []
    if char_lines:
        extras.append("CHARACTER POSITIONS THIS WEEK:\n" + "\n".join(char_lines[:6]))
    if tension_block:
        extras.append(tension_block)
