...
- `uv run pytest tests/test_scripts.py` failed even with `UV_CACHE_DIR=$TMPDIR/uv-cache`: same uv panic (3rd occurrence). Stopping further `uv run pytest` attempts until uv is usable.
...
- `python3 -m pytest tests/test_discord_review_link.py` failed: same missing `hypothesis` module from `tests/conftest.py` (3rd occurrence). Stopping further `python3 -m pytest` attempts until dependencies are installed.
