...
### 2026-09-12: Every alert that reaches Discord also reaches the inbox — severity stops routing (#7097, supersedes the routing half of #6860)

**Context:** W37's Thursday dialogue failed the judge on 2026-09-10 and paused the episode. `notify_judge_failure` sends at `severity="warning"`, and the 09-05 decision above routed only `critical` to email, so the alert went to Discord and nowhere else. Erik happened to be at his computer when it arrived; nothing about that was by design. He found out the week had not published two days later, while the Sunday stage was failing the same way. Erik: *"I don't check Discord, so sending something to Discord only should probably be nothing. If it's going to Discord, it should also go to email."*

**Decision:** `_EMAIL_SEVERITIES` is deleted. Both backends receive every alert; severity now only picks the Discord embed color and labels the email subject. No caller changed.
